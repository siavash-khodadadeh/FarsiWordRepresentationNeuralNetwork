...
neuronsStructure = [allWords.size,wordFeatureVectorSize,allWords.size];
...
%             label = convertToInputVector(label);
            trainSet(ceil(windowSize/2)+1) = [];
            trainSet(ceil(windowSize/2)) = [];
%             inputVector = convertToInputVector(trainSet);
%             inputVector = mat2cell(inputVector);
            trainSet = trainSet{1};
            trainSetIndex = -1;
            if isnan(str2double(trainSet)) == 0
                trainSetIndex = allWords.indexOf('$digits')+1;
            end
            if trainSetIndex == -1
                if length(trainSet) <= 1
                    trainSet = [trainSet,'#'];
                end
                if allWords.indexOf(trainSet) >= 0
                    trainSetIndex = allWords.indexOf(trainSet)+1;
                end
            end
            if trainSetIndex == -1
               trainSetIndex = allWords.indexOf('$notfr')+1;
            end

    %         label = convertToInputVector(label);

            trainSet = -ones(allWords.size,1);
            trainSet(trainSetIndex) = 1;


    %         label = sentenceTagging(j);
            label = mat2cell(label);
            inputVector = mat2cell(trainSet);
%             [weights] = fine_tuning(inputVector,label, neuronsStructure, learningRate, weights,iterationNumber);
            [weights] = fine_tuning_feature_change(inputVector,label, neuronsStructure, learningRate, weights,iterationNumber);
